...
const UpdateInvoiceModal = ({ invoice, onClose, onRefresh }) => {
  ...
  return (
    <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center p-2 sm:p-4 z-[100]">
      <div className="bg-white rounded-2xl w-full max-w-full sm:max-w-lg md:max-w-2xl shadow-2xl overflow-hidden border border-orange-100 animate-in fade-in zoom-in duration-200 max-h-[70vh] flex flex-col mt-[60px] sm:mt-20">
        {/* Header */}
        ...
        <form onSubmit={handleSubmit} className="p-3 sm:p-6 overflow-y-auto space-y-8 bg-white text-left max-h-[60vh] sm:max-h-[65vh]">
        ...
        <div className="p-3 sm:p-4 bg-gray-50 border-t border-gray-100 flex flex-col sm:flex-row gap-2 sm:gap-3 shrink-0 sticky bottom-0 z-10">
          <button type="button" onClick={onClose} className="w-full sm:w-40 px-4 py-2.5 text-gray-600 font-bold bg-white border border-gray-200 hover:bg-gray-100 rounded-xl transition-all">Cancel</button>
          <button onClick={handleSubmit} disabled={isSubmitting} className="w-full sm:w-40 px-4 py-2.5 bg-orange-500 text-white font-bold rounded-xl hover:bg-orange-600 transition-all shadow-lg shadow-orange-200 disabled:opacity-50">
