...
export default function EmployeePortal() {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      if (attendanceRes && attendanceRes.success) {
        ...
        } else if (todayRecord && todayRecord.punchIn && todayRecord.punchOut) {
          // 3. User already finished a session today. Show "PUNCH IN" for next session.
          setStatus("OUT");
          setSessionTime("00:00:00");
        } else {
  ...
  const handlePunchAction = async () => {
    ...
    try {
      ...
      if (status === "OUT") {
        ...
        setHasJustSubmittedFix(false);
        Swal.fire("Punched In", "Have a productive day!", "success");
      } 
      else if (status === "IN") {
        // Standard Punch Out with +5:30 adjustment
        const now = new Date();
        const adjustedTime = new Date(now.getTime() + (5.5 * 60 * 60 * 1000));
        await AttendanceService.punchOut({ punchOutTime: adjustedTime.toISOString() });
        ...
        if (formValues) {
          // Fix Log submission with +5:30 adjustment
          const userDate = new Date(formValues.requestedPunchOut);
          const adjustedPunchOut = new Date(userDate.getTime() + (5.5 * 60 * 60 * 1000));
          
          await AttendanceService.submitPunchOutRequest({
            requestedPunchOut: adjustedPunchOut.toISOString(),
      ...
      if (serverMsg === "Already sent for admin approval") {
        setHasJustSubmittedFix(true);
        setStatus("OUT");
        Swal.fire("Pending", "Request already exists.", "info");
      } else {
  ...
  return (
    <div className="min-h-screen p-4 md:p-8 md:py-20 font-sans ">
